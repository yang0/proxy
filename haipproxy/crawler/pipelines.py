...
class ProxyCommonPipeline(BasePipeline):
    ...
    def _process_score_item(self, item, spider):
        ...
        if score is None:
            if item['incr'] == '-inf' or item['incr'] < 0:
                self.redis_con.srem(DATA_ALL, item['url'])
            else:
                self.redis_con.zadd(item['queue'], {item['url']: item['score']+1} )
            # logger.debug(f"item: {item} inserted")
        else:
    ...
    def _process_speed_item(self, item, spider):
        if item['incr'] == '-inf' or item['incr'] < 0:
            # logger.warning(f'item verification has failed {item["url"]}')
            raise DropItem('item verification has failed')
