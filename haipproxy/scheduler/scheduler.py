...
from ..crawler.validators import all_validators, douban_validators
from ..config.settings import (
    SPIDER_COMMON_TASK, SPIDER_AJAX_TASK,
    SPIDER_GFW_TASK, SPIDER_AJAX_GFW_TASK,
    TEMP_HTTP_QUEUE, TEMP_HTTPS_QUEUE,TEMP_DOUBAN_QUEUE,
    TIMER_RECORDER, TTL_VALIDATED_RESOURCE)
from ..utils import (
    get_redis_conn, acquire_lock,
    release_lock)
from ..crawler.validators.douban import DoubanValidator
...
DEFAULT_VALIDATORS =  douban_validators
...
class BaseScheduler:
    ...
    def schedule_with_delay(self, test=False):
    ...
    def schedule_task_with_lock(self, tasks):
...
class ValidatorScheduler(BaseScheduler):
    def schedule_task_with_lock(self, task):
        ...
        lock_indentifier = acquire_lock(conn, task_name) # 获取锁是为了避免太多并发
        if not lock_indentifier:
            return False
        pipe = conn.pipeline(True)
        try:
...
@click.command()
@click.option('--usage', type=click.Choice(['crawler', 'validator', 'test']), default='crawler')
...
def scheduler_start(usage, task_queues):
    ...
    if usage == 'crawler' or usage == 'test':
    ...
    if not task_queues:
        ...
        for task_queue in task_queues:
            ...
            logger.info(f"allow_task_queue:{allow_task_queue}")
